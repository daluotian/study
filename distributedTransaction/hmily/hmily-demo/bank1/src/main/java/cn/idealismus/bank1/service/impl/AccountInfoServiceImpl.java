...
public class AccountInfoServiceImpl implements AccountInfoService {
    ...
    public void updateAccountBalance(String accountNo, Double amount) {
        ...
        if (accountInfoDao.isExistTry(transId) > 0) {
            System.out.println("bank1 try 已经执行...,transId: " + transId);
            return;
        }
        
        //悬挂处理
        if (accountInfoDao.isExistConfirm(transId) > 0 || accountInfoDao.isExistCancel(transId) > 0) {
            System.out.println("bank1 try 悬挂处理...,transId: " + transId);
            return;
        }
        ...
        if (amount == 2) {
            throw new RuntimeException("bank1 远程调用转账成功,人为异常开始...,transId: " + transId);
        }
    }
    ...
    public void commit (String accountNo, Double amount) {
        //获取全局事务ID
        String transId = HmilyTransactionContextLocal.getInstance().get().getTransId();
        System.out.println("bank1 confirm 已经执行...,transId: " + transId);
    }
    ...
    public void rollback (String accountNo, Double amount) {
        ...
        if (accountInfoDao.isExistConfirm(transId) > 0) {
            System.out.println("bank1 cancel 已处理，幂等，无需处理第二次...,transId: " + transId);
            return;
        }
        //做空回滚
        if (accountInfoDao.isExistTry(transId) <= 0) {
            System.out.println("bank1 cancel 还未开始try,空回滚...,transId: " + transId);
            return;
        } 
